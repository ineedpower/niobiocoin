...
#if defined(NDEBUG)
# error "Niobiocoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6875a4073e545d66af8436f57a9d46234cedee22f7d94a69299290e98d9f3774");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Niobiocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Niobiocoin Signed Message:\n";
...
{
    // Niobiocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Niobiocoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 15 * 60; // Niobiocoin: 15 minutes
static const int64 nTargetSpacing = 1.5 * 60; // Niobiocoin: 1.5 minutos
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Niobiocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Niobiocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6875a4073e545d66af8436f57a9d46234cedee22f7d94a69299290e98d9f3774");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is the future, Niobiocoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xe1107b2f2e6f17195493e7de9085d3f552a0b745ed925c477135d64a5f6a9055"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Niobiocoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NiobiocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NiobiocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NiobiocoinMiner(CWallet *pwallet)
{
    printf("NiobiocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("niobiocoin-miner");
    ...
    try { loop {
        ...
        printf("Running NiobiocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NiobiocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NiobiocoinMiner, pwallet));
}
